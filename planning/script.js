...
function undoPlanning() {
  ...
  if (currentSaved) {
    redoStack.push({ date, data: currentSaved });
  }
...
function imprimerPlanning() {
  ...
  document.querySelectorAll("#table-body tr").forEach((tr) => {
    ...
    const chantierVal = cp(tr.cells[2]?.innerText);
  ...
  sortedCells.forEach((td) => {
    ...
    if (val !== "") {
      if (input && input.classList.contains("select-telephone")) {
        ...
        else if (cellIndex === 9) val = "N° Chantier : " + val;
        else if (cellIndex === 10) val = "N° Commande : " + val;
        else if (cellIndex === 11) val = "N° Bon : " + val;
      }
    }
...
function getICClient(clientNom, matosNom, tarifManuel = null) {
  const groupeNom = transportConfigClient[clientNom];
  if (!groupeNom) return 0;
  const groupeData = transportConfigGroup[groupeNom];
  if (groupeData && groupeData[matosNom]) {
    const tarif =
      tarifManuel !== null ? tarifManuel : getTarifClient(clientNom, matosNom);
    let indice = 0;
...
function updateRowTarif(tr) {
  ...
  // On détecte si c'est une action utilisateur (clic menu) ou un calcul automatique (rafraîchissement)
  ...
  // --- SÉCURITÉ ANTI-EFFACEMENT AU RAFRAICHISSEMENT ---
  // Si au chargement la cellule contient déjà un montant (ex: 388.80),
  // on lui remet sa protection "is-manual" pour ne pas qu'elle soit écrasée.
  [cellTarifClient, cellTarifAffrete].forEach((cell) => {
    const val =
      parseFloat(cell.textContent.replace(/[€\s]/g, "").replace(",", ".")) || 0;
    if (val !== 0 && !isComboChange) {
  ...
  // --- LOGIQUE CLIENT ---
  if (client && matos) {
    if (!cellTarifClient.classList.contains("is-manual") || isComboChange) {
    ...
    const icC = getICClient(client, matos, tarifAfficheC);
    const cellICClient = tr.querySelector(".cell-IC-client");
    cellICClient.textContent = icC.toFixed(2) + " €";
    cellICClient.style.color =
      icC > 0 ? "#28a745" : icC < 0 ? "#dc3545" : "#000000";
    cellICClient.style.fontWeight = icC !== 0 ? "bold" : "normal";
  }

  // --- LOGIQUE AFFRETE ---
  if (associe && matos) {
    if (!cellTarifAffrete.classList.contains("is-manual") || isComboChange) {
      let tarifA = getTarifAffrete(associe, matos);
...
function refreshAllPlanningTarifs() {
  ...
  const rows = document.querySelectorAll("#table-body tr");
  rows.forEach((tr) => {
    const dateLigne = tr.cells[3]?.innerText || ""; // format DD/MM/YY
    const partiesDate = dateLigne.split("/");
    const anneeLigne = partiesDate.length === 3 ? "20" + partiesDate[2] : "";

    if (anneeLigne && anneeLigne !== anneeEnCours) return;
    ...
    if (client && matos) {
      const cellTarif = tr.querySelector(".cell-tarif");
      const tarifActuel =
        parseFloat(
          cellTarif.textContent.replace(/[€\s]/g, "").replace(",", "."),
        ) || 0;
      const icC = getICClient(client, matos, tarifActuel);
      if (tr.dataset.isManualTarif !== "true") {
        const tarifC = getTarifClient(client, matos);
        cellTarif.textContent = tarifC.toFixed(2) + " €";
      }
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (
      ...
      key.substring(9, 13) === anneeEnCours
    ) {
  ...
  keysToUpdate.forEach((key) => {
    try {
      ...
      dayData.forEach((row) => {
        if (row.isManualTarif) return;
        if (row.isManualTarifAffrete) return;
        const client =
...
function saveCurrentDay() {
  ...
  document.querySelectorAll("#table-body tr").forEach((tr) => {
    ...
    rows.push({
      ...
      isManualTarif: tr.cells[12].classList.contains("is-manual"), // AJOUT
      supplement_client: tr.cells[13].innerText,
      IC_client: tr.cells[14].innerText,
      tarif_affrete: tr.cells[15].innerText,
      isManualTarifAffrete: tr.cells[15].classList.contains("is-manual"), // AJOUT
      supplement_affrete: tr.cells[16].innerText,
      IC_affrete: tr.cells[17].innerText,
      commentaire: tr.cells[18].innerText,
      snapshot: snapshot,
      couleurs_cellules:
...
function estMemeMoisQueSelection(dateStr) {
  ...
  const dateSelectionnee = document.getElementById("current-date-picker").value;
  ...
  const moisSelectionne = dateSelectionnee.substring(0, 7);
...
function applySnapshotToRow(tr, snapshot) {
  ...
  if (snapshot.tarif_client_snapshot !== undefined) {
    cellTarifClient.classList.add("is-manual");
  ...
  if (snapshot.tarif_affrete_snapshot !== undefined) {
    cellTarifAffrete.classList.add("is-manual");
...
function ajouterLigne(data = null) {
  ...
  tr.querySelectorAll('[contenteditable="true"]').forEach((cell) => {
    ...
    cell.addEventListener("blur", () => {
      ...
      if (isNumericCell) {
        ...
        if (cell.classList.contains("cell-tarif") && val > 0) {
          const cellAffrete = tr.querySelector(".cell-tarif-affrete");
          const valAffrete =
            parseFloat(
              cellAffrete.textContent.replace(/[€\s]/g, "").replace(",", "."),
            ) || 0;
          if (valAffrete === 0) {
            const tarifAffrete96 = Math.round(val * 0.96 * 100) / 100;
            cellAffrete.textContent = tarifAffrete96.toFixed(2) + " €";
            cellAffrete.classList.add("is-manual");
          }
        }
        if (
          cell.classList.contains("cell-tarif") ||
          ...
          if (val > 0) cell.classList.add("is-manual");
          updateRowTarif(tr);
        }
        calculerTousLesTotaux();
  ...
  const cellC = tr.querySelector(".cell-tarif");
  const cellA = tr.querySelector(".cell-tarif-affrete");
  ...
  if (data) {
    ...
    if (data.isManualTarif) cellC.classList.add("is-manual");
    if (data.isManualTarifAffrete) cellA.classList.add("is-manual");
  } else {
    tr.style.backgroundColor = "";
    tr.removeAttribute("style");
    cells.forEach((td) => {
...
function dupliquerLigne(bouton) {
  ...
  const data = {
    ...
    tarif: ligne.cells[12].innerText,
    supplement_client: ligne.cells[13].innerText,
    IC_client: ligne.cells[14].innerText,
    tarif_affrete: ligne.cells[15].innerText,
    supplement_affrete: ligne.cells[16].innerText,
    IC_affrete: ligne.cells[17].innerText,
    commentaire: ligne.cells[18].innerText,
  };
  ajouterLigne(data);
  const tableBody = document.getElementById("table-body");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    ...
    .addEventListener("click", function () {
      const datePicker = document.getElementById("current-date-picker").value;
      if (!datePicker) {
        alert(
